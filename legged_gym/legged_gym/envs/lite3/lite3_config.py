...
class Lite3RoughCfg( LeggedRobotCfg ):
    class env( LeggedRobotCfg.env ):
        ...
        class commands( LeggedRobotCfg.commands ):
            curriculum = True
            max_curriculum = 3 #最大难度 - 限制最大速度为2m/s
    ...
    class control( LeggedRobotCfg.control ):
        ...
        stiffness = {'joint': 40.0}  # 关节刚度 [N*m/rad] - 匹配部署参数
        damping = {'joint': 1.0}     # 关节阻尼 [N*m*s/rad] - 匹配部署参数
    ...
    class rewards( LeggedRobotCfg.rewards ):
        ...
        base_height_target = 0.3  # 基座目标高度 [m] 
        max_contact_force = 100.  # 最大接触力，超过此值会被惩罚
        clearance_height_target = 0.20  # 足部离地目标高度 [m] 
        # VHIP参数
...
class Lite3RoughCfgPPO( LeggedRobotCfgPPO ):
    seed = 1
    runner_class_name = 'OnPolicyRunner'  # 使用标准训练运行器
    ...
    class runner( LeggedRobotCfgPPO.runner ):
        ...
        # resume = True  # 继续训练
        # load_run = 'Sep29_22-13-47_envppo'# 指定要加载的运行
